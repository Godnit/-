...
# Replace the rounded icosphere mountain row with actual faceted low-poly mountain meshes.
...
old_loop = "for i,(x,y,sx,sy,sz) in enumerate(mountain_blobs):\\n    ico('MountainDome_%02d'%i,(x,y,-5.0),1,M['mountain'] if i%2==0 else M['mountain_shadow'],scale=(sx,sy,sz),sub=2)\\n    ico('MountainCap_%02d'%i,(x-2.0,y-1.0,sz*0.36),1,M['snow'],scale=(sx*.34,sy*.30,sz*.18),sub=1)"
new_loop = "for i,(x,y,rx,dy,h) in enumerate(mountain_specs2):\\n    mountain_mesh('Mountain_%02d'%i,x,y,-5.0,rx,dy,h,300+i)"
if old_loop not in src:
    raise RuntimeError('V7 mountain loop target missing')
src = src.replace(old_loop, new_loop)

# Brighten only the sky/fill slightly; preserve the grass color that already matches the reference closely.
