...
set_base(BR,(0.64,0.31,0.095),.80); set_base(BL,(0.83,0.50,0.19),.80); set_base(BG,(0.985,0.955,0.885),.96)
...
def hip_tiles(prefix,rows=8,base_cols=9):
    p0=Vector((xMax,yr,zE)); p1=Vector((ridgeMax,yr,zR)); d=p1-p0; L=d.length; u=d.normalized(); a=math.atan2(d.z,d.x); rl=L/rows*.90
    for r in range(rows):
        t=(r+.47)/rows; p=p0+u*(t*L); ya=yF+(yr-yF)*t; yb=yB+(yr-yB)*t; span=max(.12,yb-ya); cols=max(2,int(round(base_cols*(1-t*.70)))); tw=span/cols*.91; stagger=.45*(span/cols) if r%2 else 0
        for c in range(cols):
            y=ya+(c+.5)*span/cols+stagger
            if y>yb-tw*.15: continue
            box(f'{prefix}_{r}_{c}',(p.x,y,p.z+.045),(rl,tw,.022),RT,.002,(0,-a,0))
hip_tiles('ExpertHipTile')
...
def gable_tiles(prefix,eave_x,rows=8,base_cols=7):
    p0=Vector((eave_x,gfront,zEg)); p1=Vector((xr,gfront,zRg)); d=p1-p0; L=d.length; u=d.normalized(); a=math.atan2(d.z,d.x); rl=L/rows*.90
    for r in range(rows):
        t=(r+.47)/rows; p=p0+u*(t*L); ymax=yF+(p.z-zE)/sm; span=max(.08,ymax-gfront); cols=max(2,int(round(base_cols*(.45+.55*t)))); tw=span/cols*.91; stagger=.44*(span/cols) if r%2 else 0
        for c in range(cols):
            y=gfront+(c+.5)*span/cols+stagger
            if y>ymax-tw*.12: continue
            box(f'{prefix}_{r}_{c}',(p.x,y,p.z+.050),(rl,tw,.022),RT,.002,(0,-a,0))
gable_tiles('ExpertGableTileL',xL); gable_tiles('ExpertGableTileR',xR)
...
for o in scene.objects:
    if o.name.startswith('TreeLeft'): o.location.y -= 1.35
...
for o in scene.objects:
    if o.type=='MESH' and (o.name.startswith('Tree') or o.name.startswith('Bush') or o.name.startswith('Flower')) and '_tr' not in o.name:
...
print('EXPERT_REFINE_V9_COMPLETE')
