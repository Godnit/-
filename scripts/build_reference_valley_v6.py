...
# These two values live in the V2 source that V5 patches, not literally in V5 itself.
needle = "ns={'__file__':str(src_path),'__name__':'__main__'}"
extra = "src=src.replace(\"CX,CY=-13.0,30.5\",\"CX,CY=-5.0,38.0\")\nsrc=src.replace(\"((x+13)/19)**2+((y-33)/11)**2\",\"((x+5)/19)**2+((y-40)/11)**2\")\n"
if needle not in src:
    raise RuntimeError('V5 execution marker missing')
